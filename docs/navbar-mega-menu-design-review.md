...
スマートフォン幅では、ロゴの左にハンバーガーボタン、右に「製品を探す」「ログイン」「カート」のアイコン操作を表示する。ハンバーガーボタンはモーダルDrawerを開き、現在は製品・ソリューション・リソースへの通常リンクを表示する。製品検索とログインはBottom sheetを開く。カートはログイン済みかつ商品ありならBottom sheetを開き、それ以外は`/cart`へ遷移する。
...
- Drawer内における`PushNav`の階層履歴、右から左へのpush遷移、戻る操作
- パネルの高度な衝突検出や自動反転
- CMSやAPIからの実データ取得
...
│     └─ MobileNavigation                  temporary direct links / Server
...
`Drawer`にも同じ配置方針を適用する。`Drawer.Root`は開閉とライフサイクル、`Drawer.Trigger`はハンバーガーボタン、`Drawer.Content`はbody Portalとネイティブ`dialog`を担当する。Drawerは階層履歴や右から左への画面遷移を知らない。これらは後続のNavbarローカルな`PushNav` Compound Componentsへ分離する。

現在の`MobileNavigation`は、モバイル用`nav`ランドマークとリンク、可視サブテキストの意味構造を担当するServer Componentである。Drawerの動作確認段階では3つの通常リンクを表示し、`PushNav`導入時にはこの意味構造の内側へ階層操作を合成する。最終的なページ遷移には`Drawer.Link`を使用し、同じpathnameを選択した場合もDrawerを明示的に閉じる。新しいタブなど別のブラウジングコンテキストを開く操作ではDrawerを維持する。
...
| `src/components/Navbar/MobileNavigation.tsx` | モバイル用`nav`、通常リンク、可視サブテキストのServer Component |
...
- [ ] Drawer内へ`PushNav`の階層履歴、push／back、画面遷移を実装する
