...
マイページとヘルプは、メガメニューを持たない通常のリンクである。メガメニュー上位リンクと共通の見た目を使用する。
...
| `NavbarActions` | コンポーネント化しない | 現状は右寄せレイアウト以外の独立責務がないため |
...
- 製品、ソリューション、リソースは「メインナビゲーション」の`nav`と`ul` / `li`
- ロゴは`nav`の外側にあるホームリンク
- ログイン、マイページ、ヘルプは「アカウントとサポート」の別`nav`
- 上位項目は`a`
- 展開領域は`role="region"`
...
| `src/components/Navbar/MegaMenu/MegaMenuRoot.tsx` | active状態、timer、位置計算、outside・route・Escape制御 |
...
| `src/components/Navbar/MegaMenu/ProductsMegaMenuContent.tsx` | 製品固有のServer Content |
| `src/components/Navbar/MegaMenu/SolutionsMegaMenuContent.tsx` | ソリューション固有のServer Content |
| `src/components/Navbar/MegaMenu/ResourcesMegaMenuContent.tsx` | リソース固有のServer Content |
