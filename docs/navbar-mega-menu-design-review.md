...
| モバイル上位項目 | `PushNav.Next`が描画する`button` | ページ遷移ではなくDrawer内の画面切り替えだから |
...
│              ├─ PushNav.Screen           root
│              │  └─ PushNav.Next × 3      category buttons
...
`MobileNavigation`は、モバイル用`nav`ランドマーク、上位項目、可視サブテキスト、カテゴリ固有Contentの意味構造を担当するServer Componentである。上位3項目は`PushNav.Next`の`button`であり、ページ遷移しない。push後の画面にカテゴリ一覧と個別ページへの最終リンクを置く。

`PushNav.Root`は値の履歴、push元要素、遷移ロック、Screen要素を管理する。`PushNav.Screen`は全画面をmountしたままtransformで移動し、activeでない画面へ`inert`と`aria-hidden`を付ける。push後は新しい画面のBackへ、back後は元のNextへfocusを移す。Drawerを閉じるとPushNav全体がunmountされるため、再度開いたときはルート画面へ戻る。
...
| `src/components/Navbar/PushNav/PushNavNext.tsx` | 次のScreenへpushする`button` |
